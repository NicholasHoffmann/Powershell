Function Rerun-Package {
    ...
    $ScheduledID = $null
    $Message = ''
    $Query = "Select * from CCM_Scheduler_ScheduledMessage where ScheduledMessageID like '" + $AdvertisementID + "%'"
    $SMSScheduledID = GWMI -Query $Query -Namespace Root\ccm\policy\machine\actualconfig -ComputerName $ComputerName
    gwmi -Query "Select * from ccm_softwaredistribution where adv_advertisementID like '${AdvertisementID}%'" -namespace 'root\ccm\policy\machine\actualconfig'
}
